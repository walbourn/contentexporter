...
ExportConsoleDialog::ExportConsoleDialog() :
    m_hRichTextBox(nullptr),
    m_hProgressText(nullptr),
...
void ExportConsoleDialog::SetProgress(float fTaskRelativeProgress)
{
    ...
    const float fAbsoluteProgress = (fTaskRelativeProgress * m_fCurrentTaskSize) + m_fCurrentTaskMin;
    const auto iProgress = static_cast<INT>(fAbsoluteProgress * 1000.0f);
