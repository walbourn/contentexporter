...
void ExportSubDProcessMesh::BuildMesh()
{
    ...
    for (size_t dwSubsetIndex = 0; dwSubsetIndex < dwSubsetCount; ++dwSubsetIndex)
    {
        ...
        {
            const DWORD dwTriangleIndex = dwSubsetBase + dwSubsetTriangleIndex;
            const INT iPolyIndex = m_pPolyMesh->GetPolygonForTriangle(dwTriangleIndex);
            ...
            const DWORD dwBaseIndex = dwTriangleIndex * 3;
            for (DWORD dwCornerIndex = 0; dwCornerIndex < 3; ++dwCornerIndex)
            {
                const INT iMeshIndex = static_cast<INT>(m_pPolyMesh->GetIB()->GetIndex(dwBaseIndex + dwCornerIndex));
                auto pPosition = reinterpret_cast<const XMFLOAT3*>(m_pPolyMesh->GetVB()->GetVertex(iMeshIndex));

                const INT iPositionIndex = CreateOrAddPosition(*pPosition, iMeshIndex);
...
INT ExportSubDProcessMesh::CreateOrAddPosition(const XMFLOAT3& vPosition, INT iMeshVertexIndex)
{
    ...
    if (iCurrentIndex == -1)
    {
        const XMVECTOR p0 = XMLoadFloat3(&vPosition);

        const size_t dwPositionCount = m_Positions.size();
        for (size_t i = 0; i < dwPositionCount; ++i)
        {
            const XMVECTOR pi = XMLoadFloat3(&m_Positions[i]);
...
void ExportSubDProcessMesh::BuildBoundaryEdgeTable()
{
    ...
    const size_t dwTotalEdgeCount = m_Triangles.size() * 3 + m_Quads.size() * 4;
    ...
    for (size_t dwTriangleIndex = 0; dwTriangleIndex < dwTriangleCount; ++dwTriangleIndex)
    {
        ...
        {
            const INT iPositionIndexA = tri.iIndices[iEdgeIndex];
            const INT iPositionIndexB = tri.iIndices[(iEdgeIndex + 1) % 3];
    ...
    for (size_t dwQuadIndex = 0; dwQuadIndex < dwQuadCount; ++dwQuadIndex)
    {
        ...
        {
            const INT iPositionIndexA = quad.iIndices[iEdgeIndex];
            const INT iPositionIndexB = quad.iIndices[(iEdgeIndex + 1) % 4];
    ...
    for (DWORD i = 0; i < dwPositionCount; ++i)
    {
        ...
        {
            const XMFLOAT3 vPos = m_Positions[i];
...
void ExportSubDProcessMesh::CreateDegenerateGeometry()
{
    ...
    while (iter != end)
    {
        ...
        const INT iPositionIndexAPrime = CreateOrAddDegeneratePosition(iPositionIndexA);
        const INT iPositionIndexBPrime = CreateOrAddDegeneratePosition(iPositionIndexB);
...
void ExportSubDProcessMesh::ComputeAdjacency()
{
    ...
    if (dwQuadCount > 0)
    {
        ...
        const float fTotalQuadCount = (float)(dwRegularQuadCount + dwExtraordinaryQuadCount);
        const float fRegularPercent = 100.0f * (float)dwRegularQuadCount / fTotalQuadCount;
...
void ExportSubDProcessMesh::ComputeTriangleAdjacency(const INT iTriangleIndex)
{
    ...
    for (INT iCornerIndex = 0; iCornerIndex < 3; ++iCornerIndex)
    {
        ...
        const INT iSweepPositionIndex = tri.iIndices[(iCornerIndex + 2) % 3];
        const INT iPivotPositionIndex = tri.iIndices[iCornerIndex];
        const INT iStopPositionIndex = tri.iIndices[(iCornerIndex + 1) % 3];
...
void ExportSubDProcessMesh::ComputeQuadAdjacency(const INT iQuadIndex)
{
    ...
    for (INT iCornerIndex = 0; iCornerIndex < 4; ++iCornerIndex)
    {
        ...
        const INT iSweepPositionIndex = quad.iIndices[(iCornerIndex + 3) % 4];
        const INT iPivotPositionIndex = quad.iIndices[iCornerIndex];
        const INT iStopPositionIndex = quad.iIndices[(iCornerIndex + 1) % 4];
...
INT ExportSubDProcessMesh::ExecuteSweep(INT iPivotPositionIndex, INT iSweepPositionIndex, INT iStopPositionIndex, INT iStartQuadIndex, INT iStartTriangleIndex, std::vector<INT>& Neighbors)
{
    ...
    const INT iStopTriangleIndex = FindTriangleWithEdge(iPivotPositionIndex, iStopPositionIndex, iStartTriangleIndex);
    const INT iStopQuadIndex = FindQuadWithEdge(iPivotPositionIndex, iStopPositionIndex, iStartQuadIndex);
    ...
    while (!bSweepComplete)
    {
        ...
        const INT iNextQuadIndex = FindQuadWithEdge(iSweepPositionIndex, iPivotPositionIndex, iCurrentQuadIndex);
        if (iNextQuadIndex != -1)
        {
            ...
            const INT iNextSweepPositionIndex = GetNextPositionIndexInQuad(iCurrentQuadIndex, iPivotPositionIndex, iSweepPositionIndex);
            ...
            if (!bFirstSweep)
            {
                ...
                const INT iOppositePositionIndex = GetOppositePositionIndexInQuad(iCurrentQuadIndex, iPivotPositionIndex);
        ...
        const INT iNextTriIndex = FindTriangleWithEdge(iSweepPositionIndex, iPivotPositionIndex, iCurrentTriangleIndex);
        if (iNextTriIndex != -1)
        {
            ...
            const INT iNextSweepPositionIndex = GetNextPositionIndexInTriangle(iCurrentTriangleIndex, iPivotPositionIndex, iSweepPositionIndex);
            assert(iNextSweepPositionIndex != -1);
    ...
    {
        const XMFLOAT3 vQuadCenter = GetQuadCenter(iStartQuadIndex);
...
XMFLOAT3 ExportSubDProcessMesh::GetQuadCenter(INT iQuadIndex)
{
    ...
    for (DWORD i = 0; i < 4; ++i)
    {
        const XMVECTOR pi = XMLoadFloat3(&m_Positions[CurrentQuad.iIndices[i]]);
...
{
    const INT iTriangleCount = static_cast<INT>(m_Triangles.size());
...
{
    const INT iQuadCount = static_cast<INT>(m_Quads.size());
    for (INT i = 0; i < iQuadCount; ++i)
    {
        ...
        {
            const INT iCornerA = quad.iIndices[iEdge];
            const INT iCornerB = quad.iIndices[(iEdge + 1) % 4];
...
INT ExportSubDProcessMesh::GetNextPositionIndexInTriangle(INT iTriangleIndex, INT iPivotPositionIndex, INT iSweepPositionIndex)
{
    ...
    for (INT i = 0; i < 3; ++i)
    {
        const INT iPositionIndex = tri.iIndices[i];
...
INT ExportSubDProcessMesh::GetNextPositionIndexInQuad(INT iQuadIndex, INT iPivotPositionIndex, INT iSweepPositionIndex)
{
    ...
    for (INT iEdge = 0; iEdge < 4; ++iEdge)
    {
        const INT iPositionIndexA = quad.iIndices[iEdge];
        const INT iPositionIndexB = quad.iIndices[(iEdge + 3) % 4];
...
void ExportSubDProcessMesh::RemoveValenceTwoQuads(std::deque<INT>& BadQuads)
{
    ...
    while (!BadQuads.empty())
    {
        ...
        const INT iQuadAIndex = BadQuads.front();
        BadQuads.pop_front();
    ...
    const size_t dwCount = QuadsForAdjacency.size();
...
void ExportSubDProcessMesh::BuildQuadPatchBuffer()
{
    ...
    for (size_t i = 0; i < dwQuadCount; ++i)
    {
        ...
        {
            const INT iPositionIndex = quad.iNeighbors[j];
            if (iPositionIndex != -1)
            {
                const INT iMeshIndex = m_PositionToMeshVertexMapping[iPositionIndex];
                assert(iMeshIndex != -1);
...
void ExportSubDProcessMesh::BuildTriPatchBuffer()
{
    ...
    for (size_t i = 0; i < dwTriangleCount; ++i)
    {
        ...
        {
            const INT iPositionIndex = Triangle.iNeighbors[j];
            if (iPositionIndex != -1)
            {
                const INT iMeshIndex = m_PositionToMeshVertexMapping[iPositionIndex];
                assert(iMeshIndex != -1);
...
void ExportSubDProcessMesh::ConvertSubsets()
{
    ...
    for (size_t i = 0; i < dwQuadCount; ++i)
    {
        ...
        const INT iMeshSubsetIndex = m_Quads[i].iMeshSubsetIndex;
    ...
    iPatchCount = 0;
    const size_t dwTriCount = m_Triangles.size();
    ...
    {
        const INT iMeshSubsetIndex = m_Triangles[i].iMeshSubsetIndex;
        if (iMeshSubsetIndex != CurrentSubset.iOriginalMeshSubset)
        {
            if (CurrentSubset.iOriginalMeshSubset != -1)
            {
...
{
    const size_t dwCount = GetSubsetCount();
