...
ExportSettingsDialog::ExportSettingsDialog() :
    m_pStaticWndProc(nullptr),
    m_hCategoryTree(nullptr),
    m_hOKButton(nullptr),
    m_hCancelButton(nullptr),
    m_hScrollingPane(nullptr),
    m_dwControlsHeight(0),
...
LRESULT CALLBACK ExportSettingsDialog::ScrollPaneWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_ERASEBKGND:
    {
        const auto dwID = static_cast<DWORD>(GetWindowLongPtr(hWnd, GWLP_ID));
...
HWND CreateCheckbox(HWND hParent, DWORD dwX, DWORD dwY, const ExportSettingsEntry* pData)
{
...
HWND CreateEditBox(HWND hParent, DWORD dwX, DWORD dwY, DWORD dwWidth, const ExportSettingsEntry* pData)
{
...
HWND CreateTrackBar(HWND hParent, DWORD dwX, DWORD dwY, DWORD dwWidth, const ExportSettingsEntry* pData)
{
...
HWND CreateTrackBarFloat(HWND hParent, DWORD dwX, DWORD dwY, DWORD dwWidth, const ExportSettingsEntry* pData)
{
    HWND hTrackBar = CreateWindowEx(0, TRACKBAR_CLASS, nullptr,
...
HWND CreateDropList(HWND hParent, DWORD dwX, DWORD dwY, DWORD dwWidth, const ExportSettingsEntry* pData)
{
...
void ExportSettingsDialog::PopulateCategories(const ExportSettingsEntry* pEntry, void* hParentItem)
{
    if (!pEntry)
    {
        ...
        const size_t dwRootCount = g_SettingsManager.GetRootCategoryCount();
    ...
    TreeItem.hParent = static_cast<HTREEITEM>(hParentItem);
...
INT GetNormalizedIntValue(const  ExportSettingsEntry* pEntry)
{
    const float fValue = pEntry->GetValueFloat();
    const float fNorm = (fValue - pEntry->m_MinValue.m_fValue) / (pEntry->m_MaxValue.m_fValue - pEntry->m_MinValue.m_fValue);
    return static_cast<INT>(fNorm * 1000.0f);
}


float GetFloatFromNormalizedInt(const ExportSettingsEntry* pEntry, INT iValue)
{
    const float fNorm = float(iValue) / 1000.0f;
    const float fValue = (fNorm * (pEntry->m_MaxValue.m_fValue - pEntry->m_MinValue.m_fValue)) + pEntry->m_MinValue.m_fValue;
    return fValue;
}
...
void ExportSettingsDialog::SynchronizeControlUI(HWND hwndControl)
{
    ...
    case ExportSettingsEntry::CT_CHECKBOX:
    {
        const bool bValue = pEntry->GetValueBool();
    ...
    case ExportSettingsEntry::CT_BOUNDEDINTSLIDER:
    {
        const INT iValue = pEntry->GetValueInt();
        SendMessage(hwndControl, TBM_SETPOS, TRUE, iValue);
        break;
    }
    case ExportSettingsEntry::CT_BOUNDEDFLOATSLIDER:
    {
        const INT iValue = GetNormalizedIntValue(pEntry);
        SendMessage(hwndControl, TBM_SETPOS, TRUE, iValue);
        break;
    }
    case ExportSettingsEntry::CT_ENUM:
    {
        ...
        const INT iCurrentValue = pEntry->GetValueInt();
...
LRESULT ExportSettingsDialog::OnMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_VSCROLL:
    {
        ...
        const INT iCurrentScrollPos = si.nPos;
    ...
    case WM_HSCROLL:
    {
        ...
        if (!pEntry)
            return false;
        const INT iPos = static_cast<INT>(SendMessage(hwndControl, TBM_GETPOS, 0, 0));
