...
void ThinDialog::CenterOnParent()
{
    ...
    const INT x = ((rcParent.right - rcParent.left) - (rcChild.right - rcChild.left)) / 2;
    const INT y = ((rcParent.bottom - rcParent.top) - (rcChild.bottom - rcChild.top)) / 2;
...
void GridLayout::SetRect(const LPCRECT rect, INT border)
{
...
void GridLayout::PlaceWindow(HWND hwnd, INT rowStart, INT colStart, INT rowSpan, INT colSpan, INT border, DWORD alignment)
{
    ...
    const INT sizeX = currentRect.right - currentRect.left;
    const INT sizeY = currentRect.bottom - currentRect.top;
